...
function add_exception($array, $type) {
    if ($type == 0) {
        $sql = pg_query('INSERT INTO exception(plc_id, prp_id, date_begin, date_end, text_excep, comments, us_id)
    VALUES (' . $array['plc'] . ', ' . $array['prp'] . ', \'' . $array['date_begin'] . '\', \'' . $array['date_end'] . '\', \'' . $array['text'] . '\', \'' . $array['comment'] . '\', ' . $_SESSION['auth']['id'] . ')');
    } else {
        $sql = pg_query('INSERT INTO exception(plc_id, prp_id, date_begin, text_excep, comments, us_id)
    VALUES (' . $array['plc'] . ', ' . $array['prp'] . ', \'' . $array['date_begin'] . '\',  \'' . $array['text'] . '\', \'' . $array['comment'] . '\', ' . $_SESSION['auth']['id'] . ')');
    }
}
